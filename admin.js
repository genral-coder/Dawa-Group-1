...
function rowHtml(t, r) {
  let cols;
  if (t === "properties") cols = [r.title_ar || r.title_en, r.location_ar || r.location_en, r.price_ar || r.price_en, r.type, r.sort];
  else if (t === "categories") cols = [r.id, r.name_ar, r.name_en, (r.images || []).length, r.sort];
  else if (t === "packages") cols = [(r.icon || "") + " " + (r.title_ar || r.title_en), r.price_ar || r.price_en, r.featured ? "✓" : "", r.sort];
  else cols = [r.name_ar || r.name_en, r.role_ar || r.role_en, r.stars, r.sort];
...
async function saveAllContent() {
  const rows = [];
  document.querySelectorAll("#contentForm textarea").forEach((ta) => {
    const k = ta.dataset.cik, lang = ta.dataset.lang;
    let r = rows.find((x) => x.key === k);
    if (!r) { r = { key: k }; rows.push(r); }
    r[lang] = ta.value;
  });
  const { error } = await sb.from("content").upsert(rows, { onConflict: "key" });
  if (error) toast("فشل الحفظ: " + error.message, false); else toast("اتحفظ كل النصوص", true);
}
